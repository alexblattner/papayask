...
const SideBar = (props: Props) => {
  const clicked = (id: string | undefined) => {
    if (id) {
      let toScrollTo = document
        .getElementById("highlight-holder")
    ...
    <div id="answer">
      ...
      {props.notes.map((note, i) => (
        <div className="note">
          {note.coordinates ? (
            <div
              onClick={() => clicked(note._id)}
              onMouseLeave={() => props.setHighlight("")}
              onMouseEnter={() => props.setHighlight(note._id)}
              className={
                "quotation" + (note._id == props.highlight ? " hover" : "")
              }
            >
              {props.description.substring(
                note.coordinates.start,
                note.coordinates.end
              )}
            </div>
          ) : (
            <></>
